...
In this article we will demonstrate how to extract data from `fvcom`
`.nc` files using similar functions to those for general `.nc`/`.nc4`
files.

## Alternative Packages

Before we demonstrate the functionality available here, we would like to
make it clear that we have not built a package to deal with all possible
aspects of an `fvcom` file. If you are looking for additional
functionality then we would recommend one of the following packages:
...
For the purpose of this example, we have collected the url for a `fvcom`
file and stored it in a separate `.txt` file and so we can load the
required packages, along with collecting the file to be read in.
...
`fvcom` files have data constructed on a triangular mesh and the stored
variables are either provided in terms of the nodes that form the bounds
of each triangle or the centroid of triangle. In the `fvcom` file these
are denoted by the `node` and `nele` dimensions respectively.
...
available and whether they are recorded at the `node` or `nele` level,
we can use
...
vignette, we first need to construct the buffers. Taking the same sites
as before:
